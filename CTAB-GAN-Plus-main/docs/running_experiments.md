...
Commands in this guide use the current thesis configurations. Set up the environment from the repository root; run Python commands from `CTAB-GAN-Plus-main`.
...
The current DP-CGANS configurations use `private=false` to compare the
non-private baseline architecture. The backend name is retained for compatibility;
these runs do not provide a differential-privacy guarantee.
...
evaluation. The current DP-CGAN runs are non-private baseline experiments.
...
Continuous-column mixture preprocessing retries non-converged fits. A fit that
still misses the tolerance is accepted only after numerical validation, with a
warning and diagnostics; unusable transformations still fail. Each GAN fit saves `training_history_A*.csv`,
...
validation, or test data. Mortality and balanced-mortality training are reported separately alongside a
real-only reference. Validation/test prevalence is unchanged in both cases.
