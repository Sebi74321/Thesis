...
SHAP therefore affects three linked components: membership of the top-$k$
feature set, the relative priorities within that set, and, indirectly, the
row-sampling probabilities. It does not assign a training record its own SHAP
value as a weight, modify the GAN loss, or update the generator through
explanation gradients. The importance vector is derived once from the common
A0 audit and reused across the weighted variants.

In particular, A3 uses SHAP alone for \emph{feature prioritisation}, not for
the complete weighting procedure. It omits the global mismatch and tail
signals from $C_j^{(\mathrm{A3})}$, but still combines the selected SHAP
priorities with the distributional region deficits $g_{jr}$ defined below.
All A2--A5 variants use this same region-deficit mechanism. A highly ranked
feature therefore does not increase a row's weight unless that row occupies
an underrepresented region. If all contributing region deficits are zero,
the weights remain one and augmentation is uniform.
...
\begin{table}[htbp]
    ...
    A2--A5 differ in feature prioritisation; all four subsequently use the
    same distributional region-deficit mechanism to construct row weights.}
    ...
    \begin{tabular}{p{13mm}p{48mm}p{68mm}}
        ...
        A2 & Mismatch-prioritised weighting & Mismatch-only feature priorities \\
        A3 & TreeSHAP-prioritised weighting & SHAP-only feature priorities \\
...
and trains a fresh GAN. Reuse is an engineering choice enabled by the
DP-CGANS saved-transformer interface: it avoids repeating the expensive
mixture fitting and keeps that model's representation fixed across variants.
It is not a requirement of differential privacy; these experiments use the
architecture in non-private mode. Rebuilding the sampler allows conditional
sampling frequencies to reflect each variant's augmented training data.

CTAB-GAN+ and CTGAN instead fit new transformers on each variant's training
table. Consequently, their ablation effects can include changes in the fitted
representation as well as changes in training-row exposure. The preprocessing
policy is therefore not identical across architectures. Comparisons describe
the implemented pipelines, not an experiment isolating the same fixed
representation for every GAN. Freezing all transformers, or explicitly
comparing frozen and refitted transformers, would be a separate control.
