...
A convincing synthetic dataset must represent more than the most common part
of a distribution. A generator may reproduce average values and frequent
categories while failing to capture extreme measurements, uncommon feature
combinations, or a minority outcome. Aggregate fidelity scores can obscure
these local failures. This is particularly problematic when synthetic data
are intended for rare-outcome prediction, because a classifier trained on
such data can only learn patterns that the generator has retained.
